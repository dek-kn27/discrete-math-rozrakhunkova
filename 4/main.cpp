...
int main() {
    ...
    unsigned long int* distances = (unsigned long int*)calloc(a->vertex_count, sizeof(unsigned long int));
    ...
    while (vertices_left) {
        ...
        for (j=0; j<a->vertex_count; j++) {
            if (!closed[j] && (a->matrix[min_vertex_i][j] != NULL)) { // перевіряємо, чи сусід і чи переглянута
                distances[j] = min(distances[min_vertex_i] + a->matrix[min_vertex_i][j]->w, distances[j]);
            };
        };
    ...
    while (vertex_i) {
        ...
        for (j = 0; j < a->vertex_count; j++) {
            if ((a->matrix[j][vertex_i] != NULL) && (distances[j] < distances[min_distance_vertex_i])) {
